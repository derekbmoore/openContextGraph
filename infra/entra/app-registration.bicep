...
@description('App ID URI (api://ctxEco-<env>)')
...
resource app 'Microsoft.Graph/applications@1.0' = {
  ...
  api: {
    ...
    oauth2PermissionScopes: [
      {
        adminConsentDescription: 'Access CtxEco API'
        adminConsentDisplayName: 'Access CtxEco API'
...
@description('Display name of the application (e.g., ctxEco-api-staging)')
param appDisplayName string

@description('Identifier URI for the API (e.g., api://ctxEco-staging)')
...
resource app 'Microsoft.Graph/applications@1.0' = {
  ...
  api: {
    ...
    oauth2PermissionScopes: [
      {
        id: newGuid()
        adminConsentDescription: 'Allow the app to access CtxEco API on behalf of the signed-in user.'
        adminConsentDisplayName: 'Access CtxEco API'
        userConsentDescription: 'Allow the app to access CtxEco API on your behalf.'
        userConsentDisplayName: 'Access CtxEco API'
        value: 'user_impersonation'
        type: 'User'
      }
    ]
...
@description('Display name prefix for the app (e.g., ctxEco-api)')
param appBaseName string = 'ctxEco-api'

@description('API application ID URI base (e.g., api://ctxEco-staging)')
param appIdUri string
    ...
    oauth2PermissionScopes: [
      {
        ...
        adminConsentDescription: 'Access the CtxEco API on behalf of the user'
        adminConsentDisplayName: 'Access CtxEco API'
        isEnabled: true
        type: 'User'
        value: 'user_impersonation'
...
@description('Identifier URI (App ID URI), e.g., api://ctxEco-staging')
param identifierUri string
