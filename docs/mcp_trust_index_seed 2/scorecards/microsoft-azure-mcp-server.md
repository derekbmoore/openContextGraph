...
**Provider:** Microsoft  
**Server / Product:** Azure MCP Server  
**Endpoint(s):** (local server)  
**Last assessed:** 2026-01-22  
**Evidence confidence:** 1 (Self-attested)  
**Trust Score:** 59.0 (**Tier:** C)  
**Enterprise fit:** Standard/Regulated (local)

---

## 1) Summary
Microsoft’s official Azure MCP server for managing Azure resources through natural-language prompts from IDEs/agents. Designed primarily as a local MCP server that uses your Azure user credentials or managed identity and enforces Azure RBAC.

## 2) Quick Flags (Unverified)
- **Auth:** Uses Azure user credentials or managed identity; secured via Azure RBAC.
- **Tool agency:** Can be highly privileged (resource creation/change). Requires strong guardrails + read-only-by-default patterns.
- **Enterprise note:** Docs caution it is intended strictly for developer use within your organization; not for external scenarios.
...
| D6 — Safety & Abuse Resistance | 5 | 2.0 | TBD |
