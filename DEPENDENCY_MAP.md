...
## Overview
This document maps all dependencies required by the voice router and related components added from engram.
...
- **Needed**: Top-level singleton like engram's `memory_client = ZepMemoryClient()`
...
- **Status**: Need to verify exact signature matches engram's
...
- **Type**: Returns `OIDCAuth` instance (ctxEco) vs `EntraIDAuth` (engram)
...
6. ❌ **SemanticContext.add_fact()** - Used for context enrichment
7. ❌ **SemanticContext.get_context_summary()** - Used for personalized instructions
8. ⚠️ **EpisodicContext.recent_turns** - Need to migrate from `recent_messages`

### Low Priority (Optional Features)
9. ❌ **webrtc_signaling_service** - Only needed for Avatar video (audio works without)
10. ❌ **agents.chat / get_agent** - May be unused (fallback for VoiceLive)
...
9. Add `SemanticContext.add_fact()` method
10. Add `SemanticContext.get_context_summary()` method

### Phase 3: Optional Features (Avatar Video)
11. Create `backend/voice/webrtc_signaling.py` (if Avatar video is needed)

### Phase 4: Settings Verification
12. Verify all `azure_voicelive_*` settings exist in `config.py`
13. Add missing settings if needed
...
- The voice router uses engram's exact pattern, so dependencies should match engram's structure
